...
do
	...
	###get Q
	arr1_${varible}=($(seq 0 $((res_${varible}-1))))
	numarr1_${varible}=${#arr1_${varible}[*]}
	res1_${varible}=${arr1_${varible}[$(($RANDOM%numarr1_${varible}))]}
	Q_raw_${varible}=${alls_${varible}[$res1_${varible}]}
	###get D
	cat Ancestors.tsv |grep -o -w "$E_raw_${varible} .*"| awk -F" " '{print $0}' |awk '{$1=null;print $0}' | tr ' ' 'n'| sort | uniq |sed '/^$/d' d_test2_${varible} >d_all_${varible}
	#selecte 10% of the descendant sequences randomly as D
	Dnum_${varible}=$[$(cat d_all_${varible} | wc -l)+10-1)/10]
	shuf d_all_${varible} -n $Dnum_${varible} -o d_select_${varible}
	seq=SEQ
	...
	for id_${varible} in `cat d_select_${varible}`
	do
		seq=SEQ
	...
	sort -n -k 2 N_q_d_all_${varible} |head -1 |awk  '{print $1}' |awk '$1~/^'$D_small_${varible}'$/'| tr  't' 'n' | tr  ' ' 'n' > d_small_sequence_${varible}
	cat e_sequence_${varible} d_small_sequence_${varible}|sort|uniq -u|awk 'END{print NR}'> N_d_e_${varible}
	cat e_sequence_${varible} q_sequence_${varible}|sort|uniq -u|awk 'END{print NR}'> N_q_e_${varible}
