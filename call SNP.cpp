...
class Nucleotide//Class of bases
{
public:
     string Reference;//Reference genomic base at the position
     int TotalDepth;//The total sequencing depth at this location is equal to A+T+C+G
     vector<int>ATCGDepth;//The respective sequencing depth of ATCG
     vector<int>ATCGForwardReads;//The forward depth of each ATCG
     vector<int>ATCGBackwardReads;//The respective reverse depth of ATCG
     string Position;
     ...
     int MainnucCount;//Number of major base reads
};

static vector<Nucleotide> RawReadcounts;//Raw Varscan Readcounts
static vector<Nucleotide> BetaCovReadcounts;//Varscan Readcounts for Beta coronavirus using Kraken results
unordered_map<string, int> ATCGMap;
bool UseBetaCov = false;//Whether to use Kraken results correction
...
1.-i readcounts file
2.-o output file
3.-k kraken readcounts file（optional）
4.-t Basenuc The lowest frequency that a major base needs to reach, default【0.7】
5.-m Minimum number of base coverage reads for make a call, default【5】
6.-Sbias Chain bias filtering parameter, default【0.1】
7.-BaseAmbiguous Whether to retain the annexing base, default【false N to replace】
*/
int main(int argc, char* argv[])
{
    printf("CallConsensusSequence v0.1:\n");
    int i, j, k;
    if (argc > 2)
    {
        for(i=1;i<argc;i+=2)
